...
public class SwipeProgressBar {
    ...
    void draw(Canvas canvas) {
        ...
        if (mRunning || (mFinishTime > 0)) {
            ...
            if (mTriggerPercentage > 0
                    && mTriggerPercentage <= 1.0
                    ) {
                mPaint.setColor(mColor1);
                drawTrigger(canvas, cx, cy);
            } else if (mTriggerPercentage > 1) {

                mPaint.setColor(deepColor(mColor1));
                drawTrigger(canvas, cx, cy);
            }
        }
        canvas.restoreToCount(restoreCount);
    ...
    private int deepColor(int color) {
        int red = Color.red(color);
        int nextRed = (int) (red + 99 * (mTriggerPercentage - 1));
        if (nextRed > 0xff) {
            red = 0xff;
        } else {
            red = nextRed;
        ...
        int nextGreen = (int) (green + 99 * (mTriggerPercentage - 1));
        if (nextRed > 0xff) {
            green = 0xff;
        } else {
            green = nextGreen;
        ...
        int nextBlue = (int) (blue + 99 * (mTriggerPercentage - 1));
        if (nextRed > 0xff) {
            blue = 0xff;
        } else {
            blue = nextBlue;
